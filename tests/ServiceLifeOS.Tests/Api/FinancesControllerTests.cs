...
public sealed class FinancesControllerTests
{
    ...
    public async Task CreateTransaction_ReturnsCreatedTransactionForAuthenticatedUser()
    {
        ...
        var controller = new FinancesController(new FinanceService(repository, new FakeAuditLogRepository(), new FakeUnitOfWork()), new FakeCurrentUser());
    ...
    private sealed class FakeFinanceRepository : IFinanceRepository
    {
        ...
        public Task<IReadOnlyCollection<FinancialCategory>> GetCategoriesAsync(string userId, bool includeArchived, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyCollection<FinancialCategory>>([]);
        ...
        public Task<FinancialTransaction?> GetTransactionAsync(string userId, Guid transactionId, CancellationToken cancellationToken = default) => Task.FromResult<FinancialTransaction?>(null);
        public Task<RecurringTransaction?> GetRecurringTransactionAsync(string userId, Guid recurringTransactionId, CancellationToken cancellationToken = default) => Task.FromResult<RecurringTransaction?>(null);
        public Task<InstallmentPurchase?> GetInstallmentPurchaseAsync(string userId, Guid installmentPurchaseId, CancellationToken cancellationToken = default) => Task.FromResult<InstallmentPurchase?>(null);
        public Task<CategoryBudget?> GetBudgetAsync(Guid categoryId, CancellationToken cancellationToken = default) => Task.FromResult<CategoryBudget?>(null);
        public Task<CategoryBudgetOverride?> GetBudgetOverrideAsync(Guid budgetId, DateOnly month, CancellationToken cancellationToken = default) => Task.FromResult<CategoryBudgetOverride?>(null);
        ...
        public Task<IReadOnlyCollection<FinancialTransaction>> GetTransactionsForInstallmentPurchaseAsync(string userId, Guid installmentPurchaseId, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyCollection<FinancialTransaction>>([]);
        public Task<IReadOnlyCollection<RecurringTransaction>> GetRecurringTransactionsAsync(string userId, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyCollection<RecurringTransaction>>([]);
        ...
        public Task AddAsync<T>(T entity, CancellationToken cancellationToken = default) where T : class { if (entity is FinancialTransaction transaction) Transactions.Add(transaction); return Task.CompletedTask; }
        public Task AddRangeAsync<T>(IEnumerable<T> entities, CancellationToken cancellationToken = default) where T : class => Task.CompletedTask;
