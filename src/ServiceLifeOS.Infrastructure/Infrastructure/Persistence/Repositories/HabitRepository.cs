...
public sealed class HabitRepository : IHabitRepository
{
    ...
    public Task<Habit?> GetHabitAsync(string userId, Guid habitId, CancellationToken cancellationToken = default) => _db.Habits.FirstOrDefaultAsync(x => x.UserId == userId && x.Id == habitId, cancellationToken);
    public async Task<IReadOnlyCollection<Habit>> GetHabitsAsync(string userId, CancellationToken cancellationToken = default) => await _db.Habits.AsNoTracking().Where(x => x.UserId == userId).ToArrayAsync(cancellationToken);
    public Task<HabitSchedule?> GetScheduleAsync(Guid habitId, CancellationToken cancellationToken = default) => _db.HabitSchedules.FirstOrDefaultAsync(x => x.HabitId == habitId, cancellationToken);
    public async Task<IReadOnlyCollection<HabitScheduleWeekday>> GetWeekdaysAsync(Guid scheduleId, CancellationToken cancellationToken = default) => await _db.HabitScheduleWeekdays.AsNoTracking().Where(x => x.HabitScheduleId == scheduleId).ToArrayAsync(cancellationToken);
    public async Task<IReadOnlyCollection<HabitCompletion>> GetCompletionsAsync(string userId, Guid habitId, CancellationToken cancellationToken = default) => await _db.HabitCompletions.AsNoTracking().Where(x => x.UserId == userId && x.HabitId == habitId).ToArrayAsync(cancellationToken);
    public Task<HabitCompletion?> GetCompletionAsync(string userId, Guid habitId, Guid completionId, CancellationToken cancellationToken = default) => _db.HabitCompletions.FirstOrDefaultAsync(x => x.UserId == userId && x.HabitId == habitId && x.Id == completionId && x.DeletedAt == null, cancellationToken);
    ...
    public async Task<IReadOnlyCollection<XpLedgerEntry>> GetXpEntriesAsync(string userId, string sourceType, CancellationToken cancellationToken = default) => await _db.XpLedgerEntries.AsNoTracking().Where(x => x.UserId == userId && x.SourceType == sourceType).ToArrayAsync(cancellationToken);
